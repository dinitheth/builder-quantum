  ...
  useEffect(() => {
    ...
    (function initGame() {
      try {
        ...
        const targetApexThreshold = targetHitRadius * 2.1;
          ...
          for (let i = vertices.length - 1; i >= 0; i--) {
            for (let ii = i - 1; ii >= 0; ii--) {
              ...
              if (compareVertices(v1, v2)) {
                vertices.splice(i, 1);
                (v2 as any).originalIndexes.push(
                  ...(v1 as any).originalIndexes,
                );
                break;
              }
            }
        ...
        const fragWireframePool = new Map(
          allColors.map((c) => [c, [] as any[]]),
        );
        const createBurst = (() => {
        ...
        function renderMenus() {
          ...
          (canvas as any).style.pointerEvents = isMenuVisible()
            ? "none"
            : "auto";
        }
        renderMenus();
        ...
        function endGame() {
          ...
          if (!isInGame()) {
            console.warn(
              "endGame() called while NOT in-game. menus.active:",
              state.menus.active,
            );
          }
